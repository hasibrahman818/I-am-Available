...
import { auth, db } from '../src/firestore/firebase.config.js';
...
const RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    ...
    const rolesArr = [];
    if (isTeacher) rolesArr.push('teacher');
    if (isStudent) rolesArr.push('student');
    ...
    try {
      ...
      await AsyncStorage.setItem('currentUser', JSON.stringify({
        ...
        photoURL,
        roles     : rolesArr,
        password // Store temporarily for demo (remove in production)
      }));
